...
import com.unshoo.pixelmusic.shared.WearCapabilities
...
class WearPhoneTransferSender @Inject constructor(
    ...
    suspend fun isPixelMusicWatchAvailable(): Boolean {
        return runCatching {
            ...
            Timber.tag(TAG).w(error, "Failed checking Saudora Wear availability")
            false
        }
    }
    ...
    suspend fun requestSongTransfer(songId: String, songTitle: String = ""): Result<Int> {
        ...
        return runCatching {
            ...
            if (nodes.isEmpty()) {
                error("No reachable watch with Saudora")
            }
